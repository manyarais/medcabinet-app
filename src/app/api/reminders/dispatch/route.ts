// POST|GET /api/reminders/dispatch — place Twilio calls for overdue doses (server-side).
// Iterates all households; each uses its own ReminderSettings + CallLog dedup.
// Auth (any one):
//   - no secret set (local/dev convenience)
//   - header x-reminder-secret: REMINDER_DISPATCH_SECRET
...
function authorize(request: NextRequest): boolean {
  const dispatchSecret = process.env.REMINDER_DISPATCH_SECRET?.trim();
  const cronSecret = process.env.CRON_SECRET?.trim();
  const headerSecret = request.headers.get("x-reminder-secret")?.trim();
  const bearer = request.headers.get("authorization")?.replace(/^Bearer\s+/i, "").trim();

  if (!dispatchSecret && !cronSecret) return true;
  if (dispatchSecret && (headerSecret === dispatchSecret || bearer === dispatchSecret)) {
    return true;
  }
...
async function runDispatch(force: boolean) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return await runDispatch(force);
  } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return await runDispatch(false);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Dispatch failed.";
    console.error("reminders/dispatch failed:", error);
    return NextResponse.json({ error: message, called: [] }, { status: 500 });
